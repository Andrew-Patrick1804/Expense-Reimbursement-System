console.log("reimbursement-resolve.js hit!!");
...
async function fillDetails(){
    ...
    /* 
        ...
        If not, then just leave them blank and add the resolve form.
    */

    if (reimbDetails.reimb_status_id != 1){
        console.log("reimb has been resolved");
        ...
        console.log("reimb has not been resolved!");

        let bodyTag = document.body;
        bodyTag.innerHTML += `<form id="resolve-form">
        <select id="status-selector">
            <option value="2">APPROVED</option>
            <option value="3">DENIED</option>
        </select>
        <button class="btn btn-success" id="resolve-btn">Resolve</button>
    </form>`;

        let resolveForm = document.getElementById("resolve-form")

        resolveForm.onsubmit = async function(e){
            e.preventDefault();

            let selectStatusElem = document.getElementById("status-selector");
            let statusId = selectStatusElem.value;
            console.log(statusId);


            let response = await fetch(`${domain}/Project1/api/reimbursement?userId=${userId}&reimbursementId=${reimbId}`,{
                method : "PATCH",
                body : JSON.stringify({
                    reimb_id : reimbId,
                    reimb_resolver_id : userId,
            ...
            if(responseData.success){
                window.location = `${domain}/Project1/manager-dashboard?userId=${userId}`;
            }
        }
    }
